...
PLA_CondRet PLA_CriarPlacar(void); //Cria um placar. Setta os pontos das equipes em 0. Retorna PLA_CondRetSemMemoria ou PLA_CondRetOk.
...
int PLA_PegarVitorias(int equipe); /* Pega as vitórias da equipe desejada. Deve-se passar 1 para as vitórias da equipe 1 e 2 para as vitórias da equipe 2.
...
int PLA_PegarPontos(int equipe); /* Pega os pontos da equipe desejada. Deve-se passar 1 para os pontos da equipe 1 e 2 para os pontos da equipe 2.
								     Caso o valor passado seja diferente de 1 e 2, ou o placar não tenha sido criado, retorna -1.*/
PLA_CondRet PLA_GuardarPontosEquipe(int equipe); /* Adiciona os pontos da rodada para uma equipe desejada. 1 para equipe 1 e 2 para a equipe 2 ou 0 para ambas.
												Retorna PLA_CondRetPlacarNaoExiste, PLA_CondRetParametroIncorreto ou PLA_CondRetOk */
PLA_CondRet PLA_GuardarVitoriasEquipe(int equipe); /* Adiciona 1 vitória para a equipe desejada. 1 para equipe 1 e 2 para a equipe 2 ou 0 para ambas.
...
int PLA_StatusRodada(void); /* Devolve o status da rodada atual. 1 se estiver corrente, 0 se não tiver uma rodada corrente ou -1 se o placar nao estiver criado.
							*/
PLA_CondRet PLA_AtualizarValorRodada(int equipe); /* Recebe a equipe que está pedindo para atualizar o valor da rodada. 1 ou 2 para a equipe 1 ou 2.
...
PLA_CondRet PLA_ChecarTruco(int equipe); /* Recebe uma equipe e verifica se o truco é possível de ser pedido ou não.
